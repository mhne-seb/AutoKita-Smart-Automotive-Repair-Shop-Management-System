-- Fix (2): start_road_test() — cancelled tasks must not count as unfinished
-- (supersedes fix_start_road_test_ambiguous_attempt_no.sql; includes that fix)
...
-- Also: tasks cancelled by an approved pull-out don't block the road test.
...
BEGIN
    ...
    WHERE spt.job_order_id = p_job_order_id AND spt.task_status NOT IN ('completed', 'cancelled');
